...
import { read } from "fs";
import Favourite, { FavouritesProps } from "../src/models/Favourites";
...
describe("User CRUD operations", () => {
    ...
    const createFavourite = async (props: Partial<FavouritesProps> = {}) => {
		return await Favourite.create(sql, {
	...
	test("Favourite was created.", async () => {
		const tagged = await createFavourite({ postId: 1, userId:1 });
    ...
    test("Favourite was retrieved.", async () => {
	 
		const fav = await createFavourite();

		 
		const readFav = await Favourite.read(sql, fav.props.userId, fav.props.postId);
	...
	test("All favourites records were retrieved", async () => {
		// Create multiple tagged records
		await createFavourite({ postId: 1, userId: 1 });
		await createFavourite({ postId: 1, userId: 2 });
		await createFavourite({ postId: 2, userId: 2 });
	
		// Retrieve all tagged records
		const allFav = await Favourite.readAll(sql);
